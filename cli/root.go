// Package cli builds the pd command tree on top of the pythondiscuss library.
package cli
...
type App struct {
	client *pythondiscuss.Client
	cfg    pythondiscuss.Config
...
func Root() *cobra.Command {
	app := &App{cfg: pythondiscuss.DefaultConfig()}

	root := &cobra.Command{
		Use:   "pd",
		Short: "Browse Python community discussions on discuss.python.org",
		Long: `pd reads the Python community Discourse forum at discuss.python.org.

All public content is available without authentication. It returns records as
table, JSON, JSONL, CSV, TSV, or URLs.

pd is an independent tool and is not affiliated with the Python Software Foundation.`,
		SilenceUsage:  true,
		SilenceErrors: true,
...
func (a *App) setup() error {
	...
	a.client = pythondiscuss.NewClient(a.cfg)
	return nil
}
