...
describes what users get. Pick a format with `-o`, or let pd choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
pd <command> -o table   # aligned columns for reading
pd <command> -o jsonl   # one JSON object per line, for piping
pd <command> -o json    # a single JSON array
pd <command> -o csv     # spreadsheet friendly
pd <command> -o tsv     # tab-separated
pd <command> -o url     # just the URL column
pd <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
pd <command> --fields id,title,url
```
...
```bash
pd <command>            # a table, because this is a terminal
pd <command> | wc -l    # JSONL, because this is a pipe
```
